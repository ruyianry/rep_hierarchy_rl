...
class DeepVAE(BaseModule):
    ...
    def __init__(
        ...
        padded_shape: Optional[Tuple] = None, # for downsampling
        ...
        **stage_kwargs,
        ...
        self.n_latents = len(config_stochastic) # number of stochastic layers
        ...
        for i, (cfg_deterministic, cfg_stochastic) in enumerate(zip(config_deterministic, config_stochastic)):
            # pick the shorter of config_deterministic and config_stochastic, and use it for the stage by the corresponding index
            top : bool = i == len(config_deterministic) - 1 # whether this is the last stage
            bottom : bool = i == 0 # whether this is the first stage
    ...
    def infer(
        ...
        posteriors = [] # list of posterior data for each stage
        input_output = self.stages[0].IO(x=x)  # Create IO struct
        for i, stage in enumerate(self.stages): # stage is either VaeStage, LvaeStage, or BivaStage
    ...
    def generate(
        ...
        **stage_kwargs,
        ...
        x_p = input_output.d # output of the last inference layer

        if self.pad is not None:  # undo padding
            x_p = nn.functional.pad(x_p, self.unpad)

        _, likelihood_data = self.likelihood(x_p=x_p, x=x) # calculate
    ...
    def forward(
        ...
        **stage_kwargs: Any,
        ...
        if save_posteriors:
            return likelihood_data, stage_datas, posteriors
        else:
            return likelihood_data, stage_datas
